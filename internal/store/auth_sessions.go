...
func (s *Store) findToken(ctx context.Context, hash, resource string, requireResource bool) (Token, []Host, error) {
	...
	hostQuery := `SELECT h.id,h.name,h.addr,h.port,h.username,h.auth_type,h.key_id,h.password_enc,h.hostkey_fp,h.jump_host_id,h.monitor_enabled,h.created_at FROM hosts h`
	hostArgs := []any{}
	if !t.AllHosts {
