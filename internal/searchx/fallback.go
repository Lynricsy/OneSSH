...
func (m *Manager) openSearchFS(ctx context.Context, host string) (searchFS, func(), error) {
...
func (m *Manager) grepSFTP(ctx context.Context, host string, opt GrepOptions) (GrepResult, error) {
	...
	defer cleanup()
	return grepFallback(ctx, fs, opt)
}

func grepFallback(ctx context.Context, fs searchFS, opt GrepOptions) (GrepResult, error) {
	pattern := opt.Pattern
	if opt.Literal {
	...
	if err != nil {
		return GrepResult{}, fmt.Errorf("Go 正则无效: %w", err)
	}
	...
	out := GrepResult{
		Lines:   make([]GrepLine, 0, min(opt.Limit, 128)),
		Engine:  "sftp",
		Warning: grepFallbackWarning,
	}
	outputBytes := len(out.Engine) + len(out.Warning) + 64
...
func (m *Manager) findSFTP(ctx context.Context, host string, opt FindOptions) (FindResult, error) {
	...
	defer cleanup()
	return findFallback(ctx, fs, opt)
}

func findFallback(ctx context.Context, fs searchFS, opt FindOptions) (FindResult, error) {
	...
	out := FindResult{
		Paths:   make([]string, 0, min(opt.Limit, 256)),
		Engine:  "sftp",
		Warning: findFallbackWarning,
	}
	outputBytes := len(out.Engine) + len(out.Warning) + 64
	walker := searchWalker{fs: fs, root: root, maxEntries: maxTraversalEntries}
...
func resolveSearchRoot(fs searchFS, name string) (string, error) {
	cwd, err := fs.Getwd()
	if err != nil {
...
func loadIgnoreRules(fs searchFS, directory, relativeDir string) ([]ignoreRule, error) {
...
func grepFallbackFile(ctx context.Context, fs searchFS, fullPath, relativePath string, info os.FileInfo, pattern *regexp.Regexp, opt GrepOptions, out *GrepResult, outputBytes *int) (bool, bool, error) {
